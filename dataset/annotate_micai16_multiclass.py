...
def extract_info_from_xml(xml_file):
    ...
    for elem in root:
        ...
        # Get details of the bounding box 
            ...
            for subelem in elem:
                if subelem.tag == "name":
                    bbox["tool"] = subelem.text
                    if c == 0: 
                        organ.append(subelem.text)
                        c +=1
                    else: tools.append(subelem.text)
                elif subelem.tag == "interaction":
                    bbox["action"] = subelem.text 
                    if c != 1: 
                        actions.append(subelem.text) 
                    c +=1                  
...
def make_annotations(organ, tools, actions, centers):
    ...
    for i in range(len(tools)):
        q = 'What is the state of ' + tools[i] + '?'
        questions.append(q)
        answers.append(actions[i])

    for i in range(len(tools)):
        q = 'Where is '+  tools[i] + ' located?'
        questions.append(q)
        x_loc = ''
        y_loc = ''
...
for subfolder in os.listdir(folder):
    ...
    ann_path = os.path.join(subfolder_path, 'annotation')
    os.mkdir(ann_path)
