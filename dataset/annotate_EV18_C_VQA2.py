...
def make_annotations(organ, tools, actions, centers):
    questions = ['Which organ is being operated?&What is the organ of interest?&What is the name of the organ?']
    answers = [organ[0]]


    for i in range(len(tools)):
        q = 'What is the state of ' + tools[i] + '?&'+ 'What is ' + tools[i] + ' doing?&' + 'What is the action of ' + tools[i] + '?'
        questions.append(q)
        answers.append(actions[i])

    for i in range(len(tools)):
        q = 'Where is '+  tools[i] + ' located?&' + 'What is the location of '+  tools[i] + '?&' 'Where is '+  tools[i] + '?'
        questions.append(q)
        x_loc = ''
        y_loc = ''
...
subfolders = [9, 10, 11, 12, 14, 15, 16]
# subfolders = [1]
...
for subfolder in subfolders:
    ...
    os.mkdir(ann_path)
    ann_path = os.path.join(subfolder_path, 'vqa2/Classification')
    os.mkdir(ann_path)
